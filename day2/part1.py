...
def main():
    ...
    for text in range(len(lines)):
        ...
        game_segment = split_line[0]
        cube_segment = split_line[1]
        valid_game_or_not = parse_cube_segment(cube_segment)
        if valid_game_or_not != "invalid" :
            game_total += int(parse_game(game_segment))
    print(game_total)
        ...
        for i in range(len(split_comma)):
            ...
            for z in range(len(split_space)):
                ...
                if (re.findall("blue",split_space[z])):
                    if number > max_blue_cubes:
                        return "invalid"
                if (re.findall("red",split_space[z])):
                    if number > max_red_cubes:
                        return "invalid"
                if (re.findall("green",split_space[z])):
                    if number > max_green_cubes:
                        return "invalid"            
